...
//PCF8574 pcf8574(0x38); // if Audio is not working then changes this address
...
uint8_t currentFanSpeed = 0; // Holds the current fan speed (0 to 3)
...
void handleFade()
{
  ...
  if (currentTime - lastUpdate < updateInterval)
    return; // Wait for the next update interval
...
void loop()
{
  ...
  case STATE_POWER_OFF:
  {
    ...
    { // Turn ON Power
      ...
      if (isAuto)
        saveStateToEEPROM(MODE_AUTO, 0);
      else
        saveStateToEEPROM(MODE_MANUAL, fanSpeed);
    }
    break;
  }
...
void set_LED_Sleep(bool b1)
{
  ...
  {
    if (b1 == true)
      gfx->drawBitmap(25, 100, sleepSign, 46, 47, foreColor);
    else
      gfx->drawBitmap(25, 100, sleepSign, 46, 47, bgColor);
  }
  if (b1 == true)
    ledStatus |= (1 << LED_SLEEP);
  else
    ledStatus &= ~(1 << LED_SLEEP);
}
...
void set_LED_Auto(bool b1)
{
  ...
  {
    if (b1 == true)
      gfx->drawBitmap(175, 95, refresh, 43, 47, foreColor);
    else
      gfx->drawBitmap(175, 95, refresh, 43, 47, bgColor);
  }
  if (b1 == true)
    ledStatus |= (1 << LED_AUTO_MODE);
  else
    ledStatus &= ~(1 << LED_AUTO_MODE);
}
...
void saveStateToEEPROM(byte mode, byte speed)
{
  if (EEPROM.read(EEPROM_ADDR_MODE) != mode)
    EEPROM.write(EEPROM_ADDR_MODE, mode);
  if (EEPROM.read(EEPROM_ADDR_SPEED) != speed)
